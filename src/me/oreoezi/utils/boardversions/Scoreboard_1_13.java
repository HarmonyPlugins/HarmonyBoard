...
public class Scoreboard_1_13 extends HarmonyScoreboard{
	private PlayerConnection connection;
	private Scoreboard scoreboard;
	private ScoreboardObjective sb_obj;
	private ArrayList<ScoreboardScore> scores;
	public Scoreboard_1_13(String name, Player player) {
		super(name, player);
    	scores = new ArrayList<ScoreboardScore>();
    	connection = ((CraftPlayer)player).getHandle().playerConnection;
    	scoreboard = new Scoreboard();
    	sb_obj = scoreboard.registerObjective(player.getName(), IScoreboardCriteria.DUMMY,IChatBaseComponent.b(Objects.requireNonNull(IChatBaseComponent.ChatSerializer.b(player.getName()))), IScoreboardCriteria.EnumScoreboardHealthDisplay.INTEGER);
